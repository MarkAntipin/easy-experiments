...
pub async fn db_create_api_key(
    ...
    sqlx::query(
        "INSERT INTO api_keys (api_key_id, company_id, name, key_hash, key_prefix, created_at)
         VALUES ($1, $2, $3, $4, $5, $6)",
    )
    .bind(&api_key_id)
    .bind(new.company_id)
...
pub async fn db_find_api_key_by_hash(
    ...
    sqlx::query_as(
        "SELECT api_key_id, company_id, name, key_hash, key_prefix, created_at
         FROM api_keys
         WHERE key_hash = $1",
    )
    .bind(key_hash)
...
pub async fn db_revoke_api_key(
    ...
    let result = sqlx::query(
        "DELETE FROM api_keys WHERE api_key_id = $1 AND company_id = $2",
    )
    .bind(api_key_id)
    .bind(company_id)
    .execute(&db.pool)
